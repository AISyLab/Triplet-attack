...
def rank_compute(prediction, att_plt, correct_key, log):
...
def rank_compute_aeshd(prediction, ciph, correct_key, log):
    hw = [bin(x).count("1") for x in range(256)]
    (nb_traces, nb_hyp) = prediction.shape
    key_log_prob = np.zeros(256)
    if log:
    ...
    for i in range(nb_traces):
        for k in range(256):
            # Computes the hypothesis values
            if nb_hyp == 256:
                key_log_prob[k] += prediction[i, AES_Sbox_inv[k^int(ciph[i,11])]^int(ciph[i,7])]
            else:
                key_log_prob[k] += prediction[i, hw[AES_Sbox_inv[k^int(ciph[i,11])]^int(ciph[i,7])]]
        rank_evol[i] = rk_key(key_log_prob, correct_key)

    return rank_evol, key_log_prob

def perform_attacks(nb_traces, predictions, correct_key, plt_attack, dataset='ASCAD', nb_attacks=50, log=True, shuffle=True, output_rank=False):
    ...
    for i in range(nb_attacks):
        ...
        if dataset == 'AESHD':
            rank_evol, key_log_prob = rank_compute_aeshd(att_pred, att_plt, correct_key, log)
        else:
            rank_evol, key_log_prob = rank_compute(att_pred, att_plt, correct_key, log)
...
def attack_calculate_metrics(model, nb_traces_attacks, correct_key, X_attack, Y_attack, plt_attack, dataset='ASCAD'):
    # Test: Attack on the test traces
    container = np.zeros((1+256+nb_traces_attacks,))
    predictions = model.predict(X_attack)
    (nb_total, nb_hyp) = predictions.shape
    # avg_rank: GE for the correct key
    # all_rank: final rank for all 256 keys
    avg_rank, all_rank = perform_attacks(nb_traces_attacks, predictions, correct_key, plt_attack, dataset=dataset, nb_attacks=10, log=True, shuffle=True, output_rank=True)
    print('GE: ', avg_rank[-1])
    print('GE smaller than 1:', np.argmax(avg_rank < 1))
    print('GE smaller than 5:', np.argmax(avg_rank < 5))
    container[257:] = avg_rank

    # calculate rank for each key
    all_key_rank = np.array(256-ss.rankdata(all_rank))
    print('Rank for each key: ', all_key_rank)
    container[1:257] = all_rank

    # calculate accuracy
    classes_lables = range(nb_hyp)
    Y_pred = proba_to_index(predictions, classes_lables)
