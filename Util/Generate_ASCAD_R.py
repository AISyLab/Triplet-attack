...
out_file_root = '/ascad-variable_4000.h5'
in_file = h5py.File('/atmega8515-raw-traces.h5', "r")
...
j = 0
for i in range(0, n_profiling + n_attack, 3):
    profiling_samples[j] = traces[i][fs:fs + ns]
    j += 2

j = 1
for i in range(0, n_profiling + n_attack, 3):
    profiling_samples[j] = traces[i + 1][fs:fs + ns]
    j += 2

j = 0
for i in range(0, n_profiling + n_attack, 3):
    attack_samples[j] = traces[i + 2][fs:fs + ns]
    j += 1
...
j = 0
for i in range(0, n_profiling + n_attack, 3):
    profiling_plaintext[j] = metadata[i][1]
    profiling_key[j] = metadata[i][0]
    profiling_masks[j] = metadata[i][2]
    j += 2
    print(i)

j = 1
for i in range(0, n_profiling + n_attack, 3):
    profiling_plaintext[j] = metadata[i + 1][1]
    profiling_key[j] = metadata[i + 1][0]
    profiling_masks[j] = metadata[i + 1][2]
    j += 2
    print(i)

j = 0
for i in range(0, n_profiling + n_attack, 3):
    attack_plaintext[j] = metadata[i + 2][1]
    attack_key[j] = metadata[i + 2][0]
    attack_masks[j] = metadata[i + 2][2]
    j += 1
    print(i)
